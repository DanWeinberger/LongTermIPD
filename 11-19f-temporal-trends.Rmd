...
eur <- eur_raw |> 
  ...
  summarise(N_IPD_st = sum(N_cases, na.rm = TRUE))|> 
  group_by(country, year) |> 
  mutate(IPD_total = sum(N_IPD_st)) |> 
  ungroup() |> 
  filter(st == "19F") |> 
  dplyr::select(-st) |> 
  rename(IPD_19F = N_IPD_st) |> 
...
#just look at proportion 19F in 2019
summary(lm(log(prop_19F_2019) ~ vax_type + time_secondvax + mean_cov + schedule, data = data_19F))

summary(lm(log(prop_19F_2019) ~ time_secondvax, data = data_19F))

#exclue US and Aus and still holds
summary(lm(log(prop_19F_2019) ~ time_secondvax, data = data_19F[!(data_19F$country %in% c('United States', 'Australia') ),]))
```
