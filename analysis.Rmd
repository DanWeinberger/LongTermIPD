...
for(i in 1:N_years){
  ...
  log(lambda[i,j]) <-  beta1[i,j]

  }
}

  for(j in 1:N_sts){
    beta1[1,j] ~ dnorm(mu[j], (1 - rho1^2) * tau.beta1) # Prior for the first time point
  for(i in 2:N_years){
    beta1[i,j] ~ dnorm(mu[j]+ rho1 * beta1[i-1,j], tau.beta1)
  }
  }
  
    rho1 ~ dunif(-1, 1)       # Uniform prior for rho; rho1 shared for all serotypes
    tau.beta1 ~ dgamma(3, 2)  # Tight prior for tau, shared for all serotypes

    for(j in 1:N_sts){
      mu[j] ~ dnorm(mu_global, tau_global)  # serotype meanscentered around global mean
      r[j] ~ dunif(0,250)

    }
    
    mu_global ~ dnorm(0,1e-4)
